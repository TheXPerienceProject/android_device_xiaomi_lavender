...
# build/make/core/config.mk:900: error: BOARD_BUILD_SYSTEM_ROOT_IMAGE cannot be true for devices with dynamic partitions.
...
QSSI_PARTITIONS := product system system_ext
TREBLE_PARTITIONS := odm vendor
ALL_PARTITIONS := $(QSSI_PARTITIONS) $(TREBLE_PARTITIONS)
$(foreach p, $(call to-upper, $(ALL_PARTITIONS)), \
    $(eval BOARD_$(p)IMAGE_FILE_SYSTEM_TYPE := ext4) \
    $(eval TARGET_COPY_OUT_$(p) := $(call to-lower, $(p))))

# Partitions dynamic retrofit
BOARD_SUPER_PARTITION_SIZE := 6747586560
BOARD_SUPER_PARTITION_GROUPS := qti_dynamic_partitions
BOARD_QTI_DYNAMIC_PARTITIONS_PARTITION_LIST := $(ALL_PARTITIONS)
BOARD_QTI_DYNAMIC_PARTITIONS_SIZE := 6743392256 # (BOARD_SUPER_PARTITION_SIZE - 4MB)
...
BOARD_SUPER_PARTITION_CUST_DEVICE_SIZE := 842006528
BOARD_SUPER_PARTITION_METADATA_DEVICE := system
QSSI_PARTITIONS_RESERVED_SIZE := 20720000

$(foreach p, $(call to-upper, $(QSSI_PARTITIONS)), \
    $(eval BOARD_$(p)IMAGE_PARTITION_RESERVED_SIZE := $(QSSI_PARTITIONS_RESERVED_SIZE)))
$(foreach p, $(call to-upper, $(TREBLE_PARTITIONS)), \
    $(eval BOARD_$(p)IMAGE_PARTITION_RESERVED_SIZE := 20720000))
...
TARGET_RECOVERY_FSTAB := $(DEVICE_PATH)/rootdir/etc/fstab_A_retro.qcom
